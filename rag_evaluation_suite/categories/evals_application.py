...
def run(rag):
    """
    Run application-level evaluation using an existing RAG pipeline.
    """
    ...
    correctness = GEval(
        ...
        rubric=[
            ...
            Rubric(
                score_range=(0, 4),
                expected_outcome=(
                    "Contains a clear factual error or a claim "
                    "that contradicts the expected output."
                ),
            ),
        ],
        evaluation_params=[
    ...
    completeness = GEval(
        ...
        rubric=[
            ...
            Rubric(
                score_range=(5, 8),
                expected_outcome=(
                    "Covers the main key points but misses one or more."
                ),
            ),
            Rubric(
                score_range=(0, 4),
                expected_outcome=(
                    "Misses several key points; only partially covers "
                    "the expected output."
                ),
            ),
        ],
    ...
    style = GEval(
        ...
        rubric=[
            Rubric(
                score_range=(9, 10),
                expected_outcome=(
                    "Clearly in a CampusX teaching voice: intuitive, "
                    "conversational prose that explains before it formalizes."
                ),
            ),
            Rubric(
                score_range=(7, 8),
                expected_outcome=(
                    "Clear, conversational, and well-explained in prose. "
                    "Fully acceptable even without an analogy or example."
                ),
            ),
            Rubric(
                score_range=(4, 6),
                expected_outcome=(
                    ...
                    "or list-heavy in places."
                ),
            ),
            Rubric(
                score_range=(0, 3),
                expected_outcome=(
                    "Dry, stiff, bare-list, jargon-heavy, or robotic; "
                    "does not read like a teaching explanation."
                ),
            ),
        ],
        evaluation_params=[
